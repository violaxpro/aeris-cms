...
export interface StockAdjustmentListType {
    // invoice_id: string
    // product_id: string
    sku: string
    qty: number
...
interface StockAdjustmentListProps {
    productForm: StockAdjustmentListType
    onChange: (form: StockAdjustmentListType) => void;
    onRemove?: () => void;
    index: number;
    length?: number
    taxType?: string
...
const StockAdjustmentList = ({
    ...
    const getGridCols = (status: string) => {
        switch (status) {
            case "Draft":
                return "md:grid-cols-[repeat(2,1fr)_200px_repeat(2,1fr)_50px]"
            default:
                return "md:grid-cols-[repeat(2,1fr)_200px_repeat(2,1fr)_50px]";
        }
    };
    ...
    return (
        <div className='flex flex-col gap-4'>
            <div className={`grid md:grid-cols-[repeat(2,1fr)_200px_repeat(2,1fr)_50px] grid-cols-2 md:gap-4 gap-6 mb-2`}>
                <SelectInput
                    id='sku'
                    label='SKU'
