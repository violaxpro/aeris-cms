...
export interface ReturnProductListType {
    // invoice_id: string
    // product_id: string
    sku: string
    reference: string
...
interface ReturnProductListProps {
    productForm: ReturnProductListType
    onChange: (form: ReturnProductListType) => void;
    onRemove?: () => void;
    index: number;
    length?: number
    taxType?: string
...
const ReturnProductList = ({
    ...
    const getGridCols = (status: string) => {
        switch (status) {
            case "Draft":
                return "md:grid-cols-[repeat(3,1fr)_200px_repeat(2,1fr)_50px]"
            case "Received":
                return "md:grid-cols-[repeat(3,1fr)_200px_repeat(3,1fr)_50px]"
            case "Inspected":
                return "md:grid-cols-[repeat(3,1fr)_200px_repeat(3,1fr)_50px]"
            case "Closed":
                return "md:grid-cols-[repeat(3,1fr)_200px_repeat(3,1fr)_50px]"
    ...
    return (
        <div className='flex flex-col gap-4'>
            <div className={`grid ${getGridCols(status)} grid-cols-2 md:gap-4 gap-6 mb-2`}>
                ...
                <SelectInput
                    id='disposition'
                    label='Desire Disposition'
                    value={productForm.disposition}
                    onChange={(val) => handleChangeSelect('disposition', val)}
                    options={[
                        { label: 'Restock', value: 'Restock' },
                        { label: 'Replace', value: 'Replace' },
                        { label: 'Refund', value: 'Refund' },
                        { label: 'Scrap', value: 'Scrap' },
                        { label: 'Return To Vendor', value: 'ReturnToVendor' },
                    ]}
                    required
                />
                <Input
