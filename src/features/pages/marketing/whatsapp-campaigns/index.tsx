...
const index = ({ inboundDatas }: { inboundDatas?: any }) => {
    ...
    const columns: TableColumnsType<any> = [
        ...
        },
        {
            title: 'Sender ID',
            dataIndex: 'sender_id',
            sorter: (a: any, b: any) => a?.sender_id.localeCompare(b?.sender_id)
        },
        {
            title: 'Audience',
            dataIndex: 'audience',
                ...
                return (
                    <div className="flex items-center justify-end gap-3 pe-4" onClick={(e) => e.stopPropagation()}>
                        <ButtonIcon
                            ...
                            size="small"
                            icon={PencilIconBlue}
                            onClick={() => router.push(routes.eCommerce.editWhatsappCampaigns(row.id))}
                        />
                        <ButtonIcon
                            color='danger'
                            variant='filled'
    ...
    return (
        <>
            ...
            <ConfirmModal
                ...
                onSave={handleDelete}
                action='Delete'
                text='Are you sure you want to delete this whatsapp campaign?'
            />
                ...
                <div className='flex justify-between items-center'>
                    ...
                    <Button
                        icon={<Image
                            ...
                            height={15}
                        />}
                        label='Add Whatsapp Campaign'
                        link={routes.eCommerce.createWhatsappCampaigns}
                    />
                </div>
            ...
            <Content className="mb-0">
                <div className='p-6 min-h-[360px'>
                    ...
                    <Table
                        columns={columns}
                        dataSource={dummyWhatsappCampaigns}
                        withSelectableRows
                        selectedRowKeys={selectedRowKeys}
                        onSelectChange={setSelectedRowKeys}
                        detailRoutes={(slug) => routes.eCommerce.detailWhatsappCampaigns(slug)}
                        getRowValue={(record) => record.id}
                    />
                    <Pagination
                        current={currentPage}
                        total={dummyWhatsappCampaigns?.length}
                        pageSize={pageSize}
