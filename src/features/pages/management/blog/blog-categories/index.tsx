...
import { BlogCategoriesType, dummyBlogCategories } from '@/plugins/types/blogs-type'
...
const index = ({ blogCategoriesData }: { blogCategoriesData?: any }) => {
    const router = useRouter()
    const { contextHolder, notifySuccess } = useNotificationAntd()
    const [data, setData] = useAtom(blogsAtom)
    const [filteredData, setFilteredData] = useState<BlogCategoriesType[]>([]);
    ...
    const columns: TableColumnsType<BlogCategoriesType> = [
        {
            title: 'Name',
            dataIndex: 'title',
            sorter: (a: any, b: any) => {
    ...
    useEffect(() => {
        setData(blogCategoriesData)
        if (!search) {
            setFilteredData(blogCategoriesData)
        }
    }, [blogCategoriesData, search])
        ...
        <>
            ...
            <div className="mt-6 mx-6 mb-0">
                <div className='flex justify-between items-center'>
                    ...
                    <Button
                        ...
                        link={routes.eCommerce.createBlogCategories}
                    />
                </div>
            </div>
            <Content className="mb-0">
                <div className='bg-[#fff] min-h-[360px] p-6'>
                    ...
                    <Table
                        columns={columns}
                        dataSource={dummyBlogCategories}
                        withSelectableRows
                    ...
                    <Pagination
                        current={currentPage}
                        total={dummyBlogCategories.length}
                        pageSize={pageSize}
