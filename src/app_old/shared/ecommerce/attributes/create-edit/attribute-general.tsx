...
import { AttributeFormInput } from '@/plugins/validators/create-attribute.schema';
import CSelect from '@/core/ui/select';
import { typeOption } from './form-utils';
import CCheckbox from '@/core/ui/checkbox';
import { useModal } from '@/app_old/shared/modal-views/use-modal';
        ...
        >
          {options?.map((item, idx) => (
            <ListboxOption
              key={idx}
              className={({ selected }) =>
                `rizzui-select-option flex w-full cursor-pointer items-center rounded-[4px] px-3 py-1.5 text-sm hover:bg-muted/70 ${selected ? 'bg-muted/70' : ''
                }`
              }
              value={item}
            >
