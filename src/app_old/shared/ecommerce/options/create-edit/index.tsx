...
import { TabButton } from '@/app_old/shared/tab-button';
import {
  OptionFormInput,
  optionFormSchema,
} from '@/plugins/validators/create-option.schema';
import OptionGeneral from './option-general';
...
export default function CreateEditOptions({ slug, className }: IndexProps) {
  const router = useRouter();
  const [isLoading, setLoading] = useState(false);
  const [tab, setTab] = useState('general');
  const [isPending, startTransition] = useTransition();

  const methods = useForm<OptionFormInput>({
  ...
  const onSubmit: SubmitHandler<OptionFormInput> = (data) => {
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
      ...
      <FormProvider {...methods}>
        ...
        >
          <div className="mb-10 grid gap-7 divide-y divide-dashed divide-gray-200 @2xl:gap-9 @3xl:gap-11">
            {tab === 'general' && <OptionGeneral />}
            {tab === 'values' && <OptionValues />}
          </div>
